package net.zerodind.minecollapsesolarcore.features;
...
public class ErosionFeature extends Feature<NoneFeatureConfiguration>
{
        ...
        for (int x = 0; x < 16; x++)
        {
            for (int z = 0; z < 16; z++)
            {
                ...
                for (int y = baseHeight; y >= minY; y--)
                {
                    ...
                    boolean stateAtIsFragile = stateAt.isAir() || MineCollapseSolarCoreFallingBlockEntity.canFallThrough(level, mutablePos, stateAt);
    ...
    private Block getHardened(Block rock) {
        if (rock == Blocks.STONE) {
            return MineCollapseSolarCore.BLOCK_HARDENED_STONE.get();
        } else if (rock == Blocks.GRANITE) {
            return MineCollapseSolarCore.BLOCK_HARDENED_GRANITE.get();
        } else if (rock == Blocks.DIORITE) {
            return MineCollapseSolarCore.BLOCK_HARDENED_DIORITE.get();
        } else if (rock == Blocks.ANDESITE) {
            return MineCollapseSolarCore.BLOCK_HARDENED_ANDESITE.get();
        } else if (rock == Blocks.TUFF) {
            return MineCollapseSolarCore.BLOCK_HARDENED_TUFF.get();
        } else if (rock == Blocks.DEEPSLATE) {
            return MineCollapseSolarCore.BLOCK_HARDENED_DEEPSLATE.get();
        }
        return null;
    }
