...
public class HorizontalSupportBlock extends VerticalSupportBlock
{
    ...
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack)
    {
        ...
        for (Direction checkDir : Direction.Plane.HORIZONTAL)
        {
            ...
            if (Helpers.isBlock(level.getBlockState(mutablePos), MineCollapse.TAG_SUPPORT_BEAMS))
            {
    ...
    public BlockState updateShape(BlockState state, Direction facing, BlockState facingState, LevelAccessor level, BlockPos currentPos, BlockPos facingPos)
    {
        ...
        {
            state = state.setValue(PROPERTY_BY_DIRECTION.get(facing), Helpers.isBlock(facingState, MineCollapse.TAG_SUPPORT_BEAMS));
            // if support incomplete, try the other way (E/W vs N/S)
            if (!Helpers.isBlock(facingState, MineCollapse.TAG_SUPPORT_BEAMS) || !Helpers.isBlock(level.getBlockState(currentPos.relative(facing.getOpposite())), MineCollapse.TAG_SUPPORT_BEAMS))
            {
                // if support incomplete here, we definitely can break
                if (!Helpers.isBlock(level.getBlockState(currentPos.relative(facing.getClockWise())), MineCollapse.TAG_SUPPORT_BEAMS)
                    || !Helpers.isBlock(level.getBlockState(currentPos.relative(facing.getCounterClockWise())), MineCollapse.TAG_SUPPORT_BEAMS))
                {
    ...
    public boolean canSurvive(BlockState state, LevelReader level, BlockPos pos)
    {
        for (Direction d : Direction.Plane.HORIZONTAL)
        {
            ...
            {
                if (Helpers.isBlock(level.getBlockState(pos.relative(d.getOpposite())), MineCollapse.TAG_SUPPORT_BEAMS))
                {
                    return true;
                }
            }
    ...
    private int getHorizontalDistance(Direction direction, LevelReader level, BlockPos pos)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (!Helpers.isBlock(stateAt, MineCollapse.TAG_SUPPORT_BEAMS) && !isEmptyOrValidFluid(stateAt))
            {
                return 0;
            }
            cursor.move(direction, 1);
            BlockState state = level.getBlockState(cursor);
            if (Helpers.isBlock(state, MineCollapse.TAG_SUPPORT_BEAMS)) // vertical only?
            {
                distance = i;
                break;
            }
