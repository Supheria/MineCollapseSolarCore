...
import net.zerodind.minecollapse.util.Collapse;
...
import net.zerodind.minecollapse.util.WorldTracker;
...
{
    public static final IndirectHashCollection<Block, CollapseRecipe> CACHE = IndirectHashCollection.createForRecipe(recipe -> recipe.getBlockIngredient().blocks(), MineCollapse.RECIPE_TYPE_COLLAPSE);
            ...
            if (realCollapse || fakeCollapse)
            {
                ...
                for (BlockPos checking : Support.findUnsupportedPositions(level, pos.offset(-radX, -radY, -radZ), pos.offset(radX, radY, radZ))) // 9x5x9 max
                {
                    ...
                    if (!checking.equals(pos) && canStartCollapse(level, checking))
                    {
                        ...
                        {
                            level.playSound(null, pos, MineCollapse.SOUND_ROCK_SLIDE_LONG.get(), SoundSource.BLOCKS, 1.0f, 1.0f);
                        }
                ...
                {
                    // Play sound
                    level.playSound(null, pos, MineCollapse.SOUND_ROCK_SLIDE_LONG_FAKE.get(), SoundSource.BLOCKS, 1.0f, 1.0f);
    ...
    public static boolean canStartCollapse(LevelAccessor level, BlockPos pos)
    {
        ...
        return Helpers.isBlock(state, MineCollapse.TAG_CAN_START_COLLAPSE)
            // If we can directly fall into this block (same as a single block collapse check), we can start a collapse
            && (MineCollapseFallingBlockEntity.canFallThrough(level, posBelow, stateBelow, Direction.DOWN, state)
            ...
            || Helpers.isBlock(stateBelow, MineCollapse.TAG_NOT_SOLID_SUPPORTING));
    }
    ...
    public static boolean startCollapse(Level level, BlockPos centerPos)
    {
        ...
        MineCollapse.LOGGER.info("Collapse started at pos {}, with the block column {} -> (start: {}) -> {}",
            centerPos,
        ...
        for (BlockPos pos : BlockPos.betweenClosed(centerPos.offset(-radius, -4, -radius), centerPos.offset(radius, -4, radius)))
        {
            ...
            for (int y = 0; y <= 8; y++)
            {
                ...
                if (foundEmpty && Helpers.isBlock(stateAt, MineCollapse.TAG_CAN_COLLAPSE))
                {
    ...
    public static boolean collapseBlock(Level level, BlockPos pos, BlockState state, boolean destroyBlockBelow)
    {
        ...
        {
            final BlockPos posBelow = pos.below();
            if (destroyBlockBelow && !MineCollapseFallingBlockEntity.canFallThrough(level, posBelow, Direction.DOWN, Blocks.BEDROCK.defaultBlockState()))
            {
            ...
            level.addFreshEntity(new MineCollapseFallingBlockEntity(level, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, collapseState, 2.0f, 20));
            return true;
        }
        return false;
    }
    ...
    {
        return MineCollapse.RECIPE_SERIALIZER_COLLAPSE.get();
    }

    @Override
    public RecipeType<?> getType()
    {
        return MineCollapse.RECIPE_TYPE_COLLAPSE.get();
    }
}
